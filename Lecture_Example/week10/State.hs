...
postOrderLabel2 :: Tree a -> State Int (Tree a)
-- postOrderLabel2 (Tree a int []) = 

postOrderLabel2 (Tree a int lst) = 
    collectM (map postOrderLabel2 lst) >>= \childs ->
        get >>= \i ->
            (let newTree = Tree a i childs
            in
                put (i+1) >>= \_ ->
                    return newTree)

sampleTree2 :: Tree String
sampleTree2 = Tree "A" 0 [
    Tree "B" 0 [],
    Tree "C" 0 [Tree "D" 0 [], Tree "E" 0 [], Tree "F" 0 []]
    ]
-- YAYYYYYYYY HAPPYYYYYYYY I MANAGE TO DO IT!!!!!! 
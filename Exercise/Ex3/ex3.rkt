...
Exercise3 SUMMARY:
* Task1: Building an environment
    * (build-env bindings)  
        * input: a datum ‘()
        * output a hash table
        * DON’T USE MUTATION! -> hash-set , no mutation, return a new hash table
* Task2: Curring
    * (curry-2 f)
        * input f: a binary function
        * output: a unary function
    * (fix-first x f)
        * input: a fix parameter x, a function that takes a list of arguments (n)
        * output: a function that takes (n-1) arguments
            * rest id*** (in lambda)
            * apply
    * (curry-n n f)
        * input: n: number of parameter, f: function that takes n parameters
        * output: a function only takes 1 parameter
        * basic idea: recursion on fix first. 
...
; task: how to write in foldl? example: '(define x 3)


(define (build-env bindings)
  (define rbindings (reverse bindings))
  (cond
    [(null? rbindings) (define hash (make-immutable-hash))
                      hash]
    [else
     (let ([first-key (car(cdr(first rbindings)))]
           [first-value (car(cdr(cdr(first rbindings))))]
           [rest-environment (build-env (rest rbindings))])
           (cond
             [(number? first-value) (hash-set rest-environment first-key first-value)]
             [else (hash-set rest-environment first-key (hash-ref rest-environment first-value))]
           )
       )
...
|#
;fix-first: return a funtion takes one less arguments than f
(define (fix-first x f)
  (lambda y
    rest y
  (apply f (append (list x) y))))
...
(define (curry-n n f)
  (cond
    [(equal? n 1) (lambda (x)
                    (f x))]
    [else (lambda (x) (curry-n (- n 1) (fix-first x f)))]
    )
  )

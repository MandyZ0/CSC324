...
{-
Exercise2 SUMMARY:
* Task1: Recursive type Definition
    * build our toy definition of list data type
    * function: numEvensList: returns the number of even items in a list   (List->Int)
        * pattern matching
    * function: mapList: same as map: (Int -> Int) -> List -> List
* Task2: find Tail call positions
    * Build our toy definition of expression
    * pattern matching
...
tailCalls (If condExpr thenExpr elseExpr) =
  merge (tailCalls thenExpr) (tailCalls elseExpr)
tailCalls (Call funcExpr args) = [Call funcExpr args]
                 -- here it's very important, you cannot directly return funcExpr, you need to return the whole things

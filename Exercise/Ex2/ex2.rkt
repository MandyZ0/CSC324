...
(define (tail-calls expr)
  (cond [(number? expr) '()]
        [(boolean? expr) '()]
        [(string? expr) '()]
        [(equal? (first expr) 'and) (tail-calls (last expr)) ]
        [(equal? (first expr) 'or) (tail-calls (last expr))]
        [(equal? (first expr) 'if) (append (tail-calls (first (rest (rest expr))))(tail-calls (last expr)))]
        [else (list expr)]
